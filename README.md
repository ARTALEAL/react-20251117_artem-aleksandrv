# Практический проект, который захватывает основные подходы разработки на React

### Инструкция

- для установки <code>npm i</code>
- где применим сервер, нужно в одном терминале запустить <code>npm run start-server</code>, в другом терминале запустить фронт <code>npm start</code>
- для запуска фронта <code>npm start</code>
